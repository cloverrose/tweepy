...
def get_users(method, **kwargs):
    ...
    if 'cursor' in kwargs:
        ...
        del kwargs['cursor']
        users = method(cursor=cursor, raw_json={'return': True, 'parse': False}, **kwargs)
    else:
        use_cursor = False
        users = method(raw_json={'return': True, 'parse': False}, **kwargs)
...
def parse_user(json):
    result = tweepy.api.get_user(raw_json={'return': False, 'parse': True, 'data': json})
    return result
